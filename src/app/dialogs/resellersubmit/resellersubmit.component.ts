...
@Component({
  selector: 'app-resellersubmit',
  templateUrl: './resellersubmit.component.html',
  styleUrls: ['./resellersubmit.component.scss']
})
export class ResellersubmitComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ResellersubmitComponent>,
    public service: AutorenewService, public globalDateFormat: globalDateFormat,
  ...
  isReseller() {
    ...
    if (accountType != null && accountType != "" && accountType.length > 0) {
      ...
      this.is_RESELLER = userType.is_RESELLER;
    }
    console.log("this.is_RESELLER ==> ", this.is_RESELLER)
  }
  ...
  onChangeCheckbox(quoteId: any, isChecked: any) {
    ...
    this.quoteDetails.forEach(element => {
      if (element.checked == true && (element.quote_sub_status == '5' || element.quote_sub_status == '7')) checkboxFlag.push(element);
    })
    let total = 0;
  ...
  selectAll(value: any) {
    if (value == true) {
      ...
      this.quoteDetails.forEach(element => {
        if (element.is_eligible === 'Y' && (element.quote_sub_status == "5" || element.quote_sub_status == "7")) element.checked = false;
        // else element.checked = false
      })
    }
  }
      ...
      data => {
        ...
        this.quoteDetailsObj.forEach(element => {
          ...
          this.quoteDetails.forEach(element => {
            ...
            if (element.is_eligible === 'Y' && (element.quote_sub_status == "4" || element.quote_sub_status == "5" || element.quote_sub_status == "7" || element.quote_sub_status == "8" || element.quote_sub_status == "10" || element.quote_sub_status == "11")) element.checked = true;
            else element.checked = false;
          });
        });
  ...
  quoteCheckBoxDisable(is_eligible: string, quote_sub_status: string) {
    if (is_eligible == 'Y' && (quote_sub_status == '5' || quote_sub_status == '7')) return false
    else return true;
  }
}
